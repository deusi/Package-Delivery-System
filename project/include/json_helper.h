...
public:
  ...
  /** @brief Creates an observer status json to be sent to an observer with OnEvent()
   *  
   * (usually as an argument in DeliverySimulation::BroadcastEvent())
   * 
   *  @param [in] status The value of the 'value' key in the JSON
  */
  static picojson::value CreateStatusJson(std::string status){
    picojson::object o = CreateJsonObject();
    AddStringToJsonObject(o, "type", "notify");
    AddStringToJsonObject(o, "value", status);
   ...
   * 
   *  @param [in] status The value of the 'value' key in the JSON
   *  @param [in] arr The path vector of the PackageTransporter
  */
  static picojson::value CreateStatusJson(std::string status, const vector<vector<float>> arr){
    picojson::object o = CreateJsonObject();
    AddStringToJsonObject(o, "type", "notify");
    AddStringToJsonObject(o, "value", status);
    o["path"] = EncodeArray(arr);
    return picojson::value(o);
  }

  /// No documentation provided. This is here to prevent warnings when running doxygen.
